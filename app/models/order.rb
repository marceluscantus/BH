...
class Order < ActiveRecord::Base
  ...
  has_many :glasses, :through => :order_items

  validates_presence_of :order_items,
                        :message => 'Tu carro está vacío! ' +
                                    'Por favor, añada algún artículo antes de procesar el pedido.'
  ...
  def process
    begin
      raise 'Una orden procesada no puede volver a abrirse' if self.closed?
      active_merchant_payment
    rescue => e
      logger.error("Orden #{id} ha fallado debido a la excepción: #{e}.")
  ...
  def active_merchant_payment
    ...
    # order information
    details = {
      :description      => 'Compra en BlackHole',
      :order_id         => self.id,
      :email            => email,
      :ip               => customer_ip,
